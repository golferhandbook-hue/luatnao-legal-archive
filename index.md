...
<div class="search-section">
  <div class="search-input-wrap">
    ...
    <input type="text" id="liveSearchInput" placeholder="Tìm kiếm theo từ khóa, mức phạt, điều luật, câu hỏi... / 키워드, 벌금, 질문 검색..." oninput="handleLiveSearch(true)" />
  </div>
...
<!-- ── 📄 3.5. 스마트 페이지네이션 컨트롤 바 ── -->
<div class="pagination-container" id="paginationNav">
  <!-- JavaScript 동적 생성 -->
</div>
...
<!-- ── 실시간 클라이언트 검색 및 고성능 페이지네이션 스크립트 ── -->
<script>
  let currentCategory = 'all';
  let currentTag = 'all';
  let currentPage = 1;
  const PAGE_SIZE = 8; // 페이지당 8개 카드 노출

  function handleLiveSearch(resetPage = false) {
    if (resetPage) {
      currentPage = 1;
    }
    ...
    if (totalItems === 0) {
      ...
      if (countBar) countBar.innerText = 'Không có kết quả nào (검색 결과 없음 0건)';
      renderPagination(0, 1);
      return;
    } else {
      if (noResults) noResults.style.display = 'none';
      if (countBar) {
        countBar.innerText = `Hiển thị ${(currentPage - 1) * PAGE_SIZE + 1} ~ ${Math.min(currentPage * PAGE_SIZE, totalItems)} / Tổng ${totalItems} bài viết (Trang ${currentPage}/${totalPages})`;
      }
    }

    // 3. 현재 페이지에 해당하는 페이지만 flex 표시
    const startIndex = (currentPage - 1) * PAGE_SIZE;
    const pagedCards = matchedCards.slice(startIndex, startIndex + PAGE_SIZE);
    pagedCards.forEach(c => c.style.display = 'flex');
  ...
  function goToPage(p) {
    currentPage = p;
    handleLiveSearch(false);
    
    const searchSection = document.querySelector('.search-section') || document.getElementById('postsGrid');
    if (searchSection) {
